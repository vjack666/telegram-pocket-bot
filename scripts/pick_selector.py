...
async def main() -> None:
    async with async_playwright() as p:
        ...
        amount = await pick(page, "MONTO")
        buy = await pick(page, "BOTON BUY")
        sell = await pick(page, "BOTON SELL")

        print("\nSelectores capturados:")
        print(f"POCKET_AMOUNT_SELECTOR={amount}")
        print(f"POCKET_BUY_SELECTOR={buy}")
        print(f"POCKET_SELL_SELECTOR={sell}")
